...
{
    public class CreadorDeModelo
    {
        private string _tensorFlowModelFilePath;
        private string _mlnetOutputZipFilePath;

        public CreadorDeModelo(string tensorFlowModelFilePath, string mlnetOutputZipFilePath)
        {
        ...
        public void Run()
        {
            ...
            var pipeline = mlContext.Transforms.ResizeImages(
                    ...
                    inputColumnName: nameof(ImagenDeEntrada.Image)
                )
            ...
            var emptyTrainingSet = mlContext.Data.LoadFromEnumerable<ImagenDeEntrada>(new List<ImagenDeEntrada>());
            ...
            var predictionEngine = mlContext.Model.CreatePredictionEngine<ImagenDeEntrada, PreddicionDeImagen>(mlModel);
            var image = (System.Drawing.Bitmap)System.Drawing.Bitmap.FromFile("../SampleImages/broccoli.jpg");
            var prediction = predictionEngine.Predict(new ImagenDeEntrada{ Image = image });
            var maxProbability = prediction.NombrePred.Max();
            var labelIndex = prediction.NombrePred.AsSpan().IndexOf(maxProbability);
